	...
	<div class="container">
	 ...
	 <div class="container">
		<ul id="myTab" class="nav nav-tabs ue-tabs">
  			<li class="active"><a href="#html" data-toggle="tab">源码</a></li>
  		</ul>
		...
		<form class="form-inline">                                       
			...
			<a class="btn ue-btn-noborder" id="moresearch">精简搜索<i class="fa fa-angle-down"></i></a>
  			<div class="btn-group pull-right">
				...
				<div class="btn-group">
					<button type="button" class="btn ue-btn dropdown-toggle" data-toggle="dropdown">更多
...
$(document).ready(function() {
	...
	$(".del").bind("click",function() {
		...
		$.dialog({
			...
			content: '确认删除该记录?',
			autofocus: true,
			ok: function(){alert("删除"+recordIds);},
			cancel: function(){}
		});
	});
	//条件查询
	$("#query").bind("click", function() {
		...
		var url = context + "/service/user/data?userName="+userName;
		grid.reload(url);
	}); 
});
...
function modify(data){
	...
	alert("修改用户ID"+recordIds);
		...
		<p><code>bootstrap.css</code>，<code>form.css</code>，<code>ui.css</code></p>
		<p style="line-height:25px">需要引用的js文件：</p>
		<p><code>bootstrap.js</code>，<code>form.js</code>，<code>ui.js</code></p>
		<h3>说明</h3>
		...
		<table class="table table-bordered">
			...
			<tbody>
				<tr>
					...
					<td>boolean</td>
					<td>true</td>
					<td>是否开启服务器模式,是否后端分页，true为后端分页，false为前端分页，默认true</td>
				</tr>
				<tr>
					<td>ordering</td>
					<td>boolean</td>
    ...
    function renderoptions(data, type, full) {
    	return '<div class="pull-center"><button type="button" class="btn btn-xs ue-btn del" style="margin-left:5px;margin-bottom:3px"><span class="fa fa-trash"></span>删除</button><button  type="button"  class="btn btn-xs ue-btn modify" style="margin-bottom:3px"><span class="fa fa-cog"></span>编辑</button></div>'
    }
    function renderstatus(data,type,full){
	...
	//修改当前记录 
	function modify(data){
		/* var url = context + "/service/ue/user/edit";
		if(data.userId){
			url += "?id=" + data.userId;
		}
